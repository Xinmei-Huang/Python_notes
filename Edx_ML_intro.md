...
#### (2) Mixture Model - Observed Case
For an observed data ($x_i$ has a specific label, not a probability)
```math 
\delta(j | i)  = 
\begin{cases}
  1 & \text{if $x_i$ is assigned to j} \\
  0 & \text{otherwise} ,
\end{cases}
```
we can compute:  

-number of observations in each cluster: $\hat{n_j} = \Sigma_{i=1}^n \delta (j | i)$. \
-p: $\frac{\hat{n_j}}{n}$. \
-mu: $\mu_j = \frac{1}{\hat{n_j}} \Sigma_{i=1}^n \delta(j | i) x_i$.\
-sigma: $\sigma_j = \frac{1}{\hat{n_j} d} \Sigma_{i=1}^n \delta (j | i) ||x_i - \mu_j||^2$.
 

#### (3) The Expectation Maximization (EM) Algorithm
We observe  data points $x_1, ..., x_n$ in $\mathbb{R}^d$. We wish to maximize the GMM likelihood with respect to the parameter set $\theta = \{ p_1, ..., p_K, \mu_1, ..., \mu_K, \sigma_1^2, ..., \sigma_K^2 \}$. Maximizing the log-likelihood $\log{\prod\limits_{i=1}^n p(x_i | \theta)}$ is not tractable in the setting of GMMs. There is no closed-form solution to finding the parameter set $\theta$ that maximizes the likelihood. The EM algorithm is an iterative algorithm that finds a locally optimal solution $\hat{\theta}$ to the GMM likelihood maximization problem.
 
1)***E-setp*** \
The E Step of the algorithm involves finding the posterior probability that point $x_i$ was generated by cluster j, for every $i = 1,...,n$ and $j=1,...,K$. This step assumes the knowledge of the parameter set $\theta$. We find the posterior using the following equation:
```math 
 p(\text{point } x_i \text{ was generated by cluset } j|x_i, \theta = p(j|i) = \frac{p_j \mathcal{N} (x_i; \mu_j, \theta_j^2 I)}{p(x_i | \theta)}
```
 
 
1)***M-setp*** \
The M Step of the algorithm maximizes a proxy function $\hat{l} (x_1, ..., x_n | \theta)$ of the log-likelihood over $\theta$, where
```math 
 \hat{l} (x_1, ..., x_n | \theta) = \Sigma_{i=1}^n  \Sigma_{j=1}^K p(j|i) \log{\frac{p(x_i \text{ and } x_i \text{ generated by cluster } j | \theta)}{p (j|i)}} 
```
...
```math 
 \hat{\mu}_j = \frac{\Sigma_{i=1}^n p (j|i) x_i}{\Sigma_{i+1}^n p (j|i)} \\
```
 
```math
 \hat{p}_j = \frac{1}{n} \Sigma_{i=1}^n p (j|i) \\
```

```math
 \sigma_j^2 = \frac{\Sigma_{i=1}^n p (j|i) ||x_i - \mu_j||^2}{d \Sigma_{i+1}^n p (j|i)}
```
 
 
The E and M steps are repeated iteratively until there is no noticeable change in the actual likelihood computed after M step using the newly estimated parameters or if the parameters do not vary by much. (If $|L_{new} - L_{old}| < \epsilon$, stop.)

 
 **Computation**: E-step of the algorithm takes $O(nkd)$, computations and the M-step of the algorithm is also of the order $O(nkd)$.
 
  
 
 #### (4) Gaussian Mixtures Models for Matrix Completion
 X is the rating matrix of d movies from n users: $X \in \mathbb{R}^{n \times d}$, where ratings are {1, ..., 5}, misisng information are filled with 0. 
```math 
X = 
\begin{bmatrix}
1 & 4 & 0 & 0 & \cdots & 5 \\
...
Set $C_u$ = \{Column index of users that ratings are observed\}, e.g., $C_1 =$ \{ $1, 2, ..., d$ \}, $X_{C_1}^{1} = (1, 4, ..., 5)$.

First clusting $x_u$ (each user) to {1, ..., K} cluster (using only observed data to estimate likelihhod), and then use the $\hat{\mu_j}$ (e.g., for user 1 belongs to cluster 3, use $\hat{\mu}^4_{j=2}, \hat{\mu}^5_{j=2}$) to represent the missing value of each $x_u$.



#### (5) EM algorithm for Matrix Completion
1)***E Setp***: posterior probability 
```math 
  p(j|u) = \frac{p_j \mathcal{N} (x^u_{C_u}; \mu^j_{C_u}, \theta^2_{j} I_{|C_u| \times |C_u|})}{\Sigma^K_{j =1} p(x^u | \theta) p_j}
```

2)***M Setp***: estimated likelihood 
```math 
 \hat{l} (x | \theta) = \Sigma_{i=1}^n  \Sigma^K_{j=1} p(j|i) \log{\frac{p(x_u \text{ and } x_u \text{ generated by cluster } j | \theta)}{p (j|u)}} 
```

```math 
 p(x_u \text{ and } x_u \text{ generated by cluster } j | \theta) = p_j \mathcal{N} (x^u_{C_u}; \mu^j_{C_u}, \theta^2_{j} I_{|C_u| \times |C_u|})
```
